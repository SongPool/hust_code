...
class SegFormer_Segmentation(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"        : "logs/without_MSFF_atten2/cross01/best_epoch_weights.pth",
        #----------------------------------------#
        #   所需要区分的类的个数+1
        #----------------------------------------#
        "num_classes"       : 2,
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        if self.mix_type == 0:
            ...
            ss = np.array(np.stack((seg_img, pr, seg_img), axis=-1), dtype=np.uint8)
            # ss = np.array(np.stack((pr, pr, pr), axis=-1), dtype=np.uint8)
