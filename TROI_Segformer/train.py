...
if __name__ == "__main__":
    start_one = time.time()
    cross_dirs = ['cross01', 'cross02', 'cross03', 'cross04', 'cross05']
    fus_atten = 2
    # for fus_atten in range(4):
    if fus_atten == 2:
        for cross_dir in cross_dirs:
            ...
            #------------------------------------------------------------------#
            save_dir            = f'logs/ours_ti_0220/atten{fus_atten}/{cross_dir}'   # 2600s一个模型，一共要（3{标签} + 5{注意力} + 5{对比}）* 5 = 65个模型 共169000s  约47个小时 焯！
            #------------------------------------------------------------------#
            ...
            model   = SegFormer(num_classes=num_classes, phi=phi, pretrained=pretrained, atten_index=fus_atten)
            ...
            with open(os.path.join(VOCdevkit_path, f"VOC2007/cross/{cross_dir}/ImageSets/Segmentation/train.txt"),"r") as f:
                train_lines = f.readlines()
            with open(os.path.join(VOCdevkit_path, f"VOC2007/cross/{cross_dir}/ImageSets/Segmentation/val.txt"),"r") as f:
        ...
        print('5 cross cost time:', time.time()-start_one)